...
function hndlLoad() {
    ...
    let numpieces;
    function rotateVector(_v, _angle) {
    ...
    const v = { x: 2, y: 0 }; // Startvektor nach rechts
    // const rotated = rotateVector(v, Math.PI / 2); // 90° Drehung
    // console.log(rotated);
    function rotateAroundPoint(p, center, _angle) {
        const translated = { x: p.x - center.x, y: p.y - center.y };
        const rotated = rotateVector(translated, _angle);
        console.log(rotated);
        return {
